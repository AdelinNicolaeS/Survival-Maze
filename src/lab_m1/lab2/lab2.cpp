...
void Lab2::Init()
{
    ...
    // Create a mesh box using custom data
    {
        vector<VertexFormat> vertices
        {
            ...
            VertexFormat(glm::vec3(-1, -1,  3), glm::vec3(1, 0, 0)),	// 2
            VertexFormat(glm::vec3(1, -1,  3), glm::vec3(1, 0, 0)),	// 3
            VertexFormat(glm::vec3(-1, -3,  1), glm::vec3(1, 0, 0)),	// 4
            VertexFormat(glm::vec3(1, -3,  1), glm::vec3(1, 0, 0)),	// 5
            VertexFormat(glm::vec3(-1, -3,  3), glm::vec3(1, 0, 0)),	// 6
            VertexFormat(glm::vec3(1, -3,  3), glm::vec3(1, 0, 0)),	// 7
    ...
    }
    {
        vector<VertexFormat> vertices
        {
            VertexFormat(glm::vec3(-0.5, -0.5, -0.5), glm::vec3(1, 0, 0)),  // 0
            VertexFormat(glm::vec3(0.5, -0.5,  -0.5), glm::vec3(1, 0, 0)),	// 1
            VertexFormat(glm::vec3(0.5, 0.5,  -0.5), glm::vec3(1, 0, 0)),	// 2
            VertexFormat(glm::vec3(-0.5, 0.5,  -0.5), glm::vec3(1, 0, 0)),	// 3
            VertexFormat(glm::vec3(-0.5, -0.5, 0.5), glm::vec3(1, 0, 0)),   // 4
            VertexFormat(glm::vec3(0.5, -0.5,  0.5), glm::vec3(1, 0, 0)),	// 5
            VertexFormat(glm::vec3(0.5, 0.5,  0.5), glm::vec3(1, 0, 0)),	// 6
            VertexFormat(glm::vec3(-0.5, 0.5,  0.5), glm::vec3(1, 0, 0)),	// 7
            // TODO(student): Complete the vertices data for the cube mesh
        ...
        meshes["cube_C"]->InitFromData(vertices, indices);
    ...
    meshes["tetra"]->InitFromData(vertices_tetra, indices_tetra);
    ...
    vector<VertexFormat> vertices_sq
    {
        ...
        VertexFormat(glm::vec3(1, 2,  0), glm::vec3(1, 1, 1)),
    ...
    meshes["square"]->InitFromData(vertices_sq, indices_sq);
    ...
    vector<VertexFormat> vcircle;
    vector<unsigned int> icircle;

    vcircle.emplace_back(glm::vec3(0, 0, 0), glm::vec3(0, 1, 2)); // centru
    double angle;
    for (int i = 0; i < 100; i++) {
        angle = 2 * M_PI / 100;
        angle *= i;
        vcircle.emplace_back(glm::vec3(cos(angle), sin(angle), 0), glm::vec3(0, 1, 2));
        icircle.push_back(i);
    }
    icircle.push_back(100);
    icircle.push_back(1);

    // Create a new mesh from buffer data
    CreateMesh("circle", vcircle, icircle);
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    //RenderMesh(meshes["cube_B"], shaders["VertexColor"], glm::vec3(-1.5f, 1.9f, -1), glm::vec3(0.25f));
    ...
    //RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(-1, 1, 0), glm::vec3(0.5f));

    //// TODO(student): Disable face culling
    //glDisable(GL_CULL_FACE);

    //RenderMesh(meshes["circle"], shaders["VertexColor"], glm::vec3(1.5f, 1, -2), glm::vec3(0.7f));

   // RenderMesh(meshes["cube_A"], shaders["VertexColor"], glm::vec3(-1.5f, 0.5f, 0), glm::vec3(0.25f));
    RenderMesh(meshes["cube_C"], shaders["VertexColor"], glm::mat4(1));
}
