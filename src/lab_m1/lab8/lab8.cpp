...
{
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(0.4, 0.8, 0.7));
...

    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, glm::vec3(0.9, 0.6, 0.8));
    ...
    // Render ground
    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, glm::vec3(0.8, 0.4, 0));
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        {
            cut_angle += deltaTime * HELPER;
            cut_angle = cut_angle > 360.f ? 360.f : cut_angle;
        }
        if (window->KeyHold(GLFW_KEY_T))
        {
            cut_angle -= deltaTime * HELPER;
            cut_angle = cut_angle < 0.f ? 0.f : cut_angle;
        }

        glm::mat4 turn = glm::mat4(1);
        turn = glm::rotate(turn, OY_angle, glm::vec3(0, 1, 0));
        turn = glm::rotate(turn, OX_angle, glm::vec3(1, 0, 0));
